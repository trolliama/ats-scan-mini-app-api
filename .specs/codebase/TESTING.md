...
- `tests/unit/` — pure unit tests (when added)
...
| `ScanRepository.get_by_id` | — | Call `get_by_id` directly (it *is* the behaviour under test) |
...
**Location:** `tests/unit/` (planned).

### Integration Tests

**Approach:** pytest with in-memory SQLite (`StaticPool`) and SQLAlchemy session fixtures.
**Location:** `tests/integration/`
**Example:** `tests/integration/test_scan_repository.py` — 8 tests exercising `ScanRepository` lifecycle (insert, duplicate, list, mark processing/completed/failed, webhook flags).
**Fixtures:** `tests/integration/conftest.py` provides `db_session` (creates schema via `init_db`, yields session, disposes engine).
**Assertion path:** Repository write/lifecycle tests verify rows with `_fetch_scan` (raw `SELECT *`); only tests whose behaviour *is* the read API call `get_by_id` / `list_incomplete` directly.
...
**Current:** Integration test suite written for `ScanRepository`; no coverage tooling configured.
**Goals:** Defined per task in `.specs/features/ats-scan-worker/tasks.md`.
**Enforcement:** None (no CI).

## Test Coverage Matrix

| Code Layer | Required Test Type | Location Pattern | Run Command |
|------------|-------------------|------------------|-------------|
| DB repositories | integration | `tests/integration/test_<repo>.py` | `poetry run pytest tests/integration/` |
| Pydantic schemas | unit | `tests/test_schemas.py` (planned) | `poetry run pytest tests/test_schemas.py` |
| HTTP routes (`/scans`) | integration | `tests/test_scans_route.py` (planned) | `poetry run pytest tests/test_scans_route.py` |
| Resume parser | unit | `tests/unit/test_resume_parser.py` (planned) | `poetry run pytest tests/unit/` |
| Agent | unit (mocked LLM) | `tests/test_agent.py` (planned) | `poetry run pytest tests/test_agent.py` |
| S3 client | unit (mocked boto3) | `tests/test_s3.py` (planned) | `poetry run pytest tests/test_s3.py` |
| Webhook client | unit | `tests/test_webhook.py` (planned) | `poetry run pytest tests/test_webhook.py` |
| Scan pipeline | integration | `tests/test_process_scan.py` (planned) | `poetry run pytest tests/test_process_scan.py` |
| Startup recovery | integration | `tests/test_recovery.py` (planned) | `poetry run pytest tests/test_recovery.py` |

## Parallelism Assessment

| Test Type | Parallel-Safe? | Isolation Model | Evidence |
|-----------|---------------|-----------------|----------|
| Integration (DB) | Yes | Fresh in-memory SQLite engine per `db_session` fixture | `conftest.py` — `StaticPool`, engine disposed after each test |
| Unit (future) | Yes | Pure functions, mock external deps | `resume_parser.py` singleton is read-only after init |
| Route tests (future) | Yes | TestClient with dependency overrides; fresh DB per test | Planned in tasks — mock S3/agent/webhook |
...
- `ScanRepository` tests written but implementation is stub — gate fails until task complete
- No unit test suite yet (`tests/unit/`)
- No CI pipeline (`.github/` absent)
